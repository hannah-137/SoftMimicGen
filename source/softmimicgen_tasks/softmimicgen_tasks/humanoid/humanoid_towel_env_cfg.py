...
from isaaclab.assets import ArticulationCfg, AssetBaseCfg, DeformableCfg
...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    object = DeformableCfg(
            prim_path="{ENV_REGEX_NS}/Object",
            init_state=DeformableCfg.InitialStateCfg(pos=[0.0, 0.5, 1.06], rot=[0.7071, 0, 0, 0.7071]),
            spawn=UsdFileCfg(usd_path=f"{SOFTMIMICGEN_ASSETS_DATA_DIR}/Props/Cloth/Cloth_crumpled.usd",)
        )
...
class TerminationsCfg:
    ...
    success = DoneTerm(func=mdp.object_flat)
...
@configclass
class GR1T2TowelEnvCfg(ManagerBasedRLEnvCfg):
    """Environment configuration for Humanoid - Towel task."""
